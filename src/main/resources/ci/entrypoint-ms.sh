#!/bin/bash
    ...
    export SPRING_DATASOURCE_USERNAME="${MARATHON_SERVICE_NAME}"
    ...
    openssl pkcs8 -topk8 -inform pem -in /etc/stratio/${MARATHON_SERVICE_NAME}.key -outform der -nocrypt -out ${POSTGRES_KEY}
    ...
    CERTIFICATE_VAR_PASS=${MARATHON_SERVICE_NAME//-/_}"_keystore"
    ...
    -in /etc/stratio/"${MARATHON_SERVICE_NAME}".pem \
    -inkey /etc/stratio/"${MARATHON_SERVICE_NAME}".key \
    -passout pass:${SPRING_KAFKA_SSL_KEYSTORE_PASSWORD}


    getCAbundle "/data/resource" "PEM" \
